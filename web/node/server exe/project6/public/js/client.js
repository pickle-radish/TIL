$(document).ready(function(){
    ...
    $(document).on('click','#login_btn',function(){
        ...
        $.post('login',send_param,function(returnData){
            ...
            if(returnData.resultCode){
                ...
                logout_form += '<hr><input type="checkbox" name="car" value="sonata">소나타</input>';
                logout_form += '<input type="checkbox" name="car" value="benz">벤츠</input>';
                logout_form += '<input type="checkbox" name="car" value="audi">아우디</input>';1
                logout_form += '<input type="checkbox" name="car" value="Genesis">제네시스</input>';
                logout_form += '<input type="button"  id="basketCar_btn" value="장바구니에 차 넣기"><br>';   
    ...
    $(document).on('click','#basketCar_btn',function(){
        let car=[];
        ...
        $.post('basketCar',send_param,function(returnData){
            alert(returnData.message);
        });
        // let car="";
        //  $("input[name=car]:checked").each(function() { 
